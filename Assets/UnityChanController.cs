...
public class UnityChanController : MonoBehaviour
{
    ...
    //左右の移動できる範囲（追加）
    private float movableRange = 49f;
    //動きを減速させる係数（追加）
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.Space) || this.isJButtonDown) && this.transform.position.y < 1.5f)
            {
                //ジャンプアニメを再生（追加）
    ...
    void OnCollisionEnter(Collision other)
    {
        ...
        //障害物に衝突した場合（追加）
        if (other.gameObject.tag == "BombBallTag" || other.gameObject.tag == "EyeBallTag")
        {
            this.isEnd = true;
        ...
        if (other.gameObject.tag == "SpikeBallTag")
        {
            this.isEnd = true;
            //stateTextにGAME OVERを表示（追加）
            this.stateText.GetComponent<Text>().text = "GAME OVER";
